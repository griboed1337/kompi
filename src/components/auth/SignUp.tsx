  ...
  return (
    <Card className="w-full max-w-md border-none shadow-2xl bg-card/50 backdrop-blur-md overflow-hidden ring-1 ring-border/50">
      ...
      <CardHeader className="space-y-1 p-6">
        <CardTitle className="text-2xl font-black">Регистрация</CardTitle>
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4 p-6 pt-0">
          {error && (
            <div className="p-3 bg-destructive/10 text-destructive border border-destructive/20 rounded-xl text-sm font-medium">
              {error === 'Passwords do not match' ? 'Пароли не совпадают' : error}
            </div>
          )}
          ...
          <div className="space-y-2">
            <Label htmlFor="confirmPassword" title="Подтвердите пароль" className="font-semibold text-sm">Подтвердите пароль</Label>
            <Input
              id="confirmPassword"
              type="password"
        ...
        <CardFooter className="flex flex-col p-8 pt-8">
          <Button
          ...
          <div className="mt-6 text-center text-sm text-muted-foreground font-medium">
            Уже есть аккаунт?{' '}
            <a href="/auth/signin" className="text-primary hover:underline font-bold transition-all">
              Войти
            </a>
          </div>
        </CardFooter>
