...
Важно: Обязательно добавляй объект "specs" с 3-5 ключевыми характеристиками, релевантными для категории. Отвечай строго в формате JSON без пояснений. 
ВНИМАНИЕ: Сейчас ФЕВРАЛЬ 2026 года. Цены на RAM и SSD выросли в 2-3 раза по сравнению с 2025 годом. 
Ориентировочные цены на сегодня: 16GB DDR4 ~10-15k ₽, 16GB DDR5 ~20-30k ₽, 1TB NVMe ~15-20k ₽. Процессоры и видеокарты также подорожали на 30-50%.`;

const HARDWARE_PRICES_PROMPT = `Ты — эксперт по ценам на ПК комплектующие. 
Твоя задача: обновить ЦЕНЫ для конкретного списка моделей в категории: {category}.
Данные предоставлены в формате: [Название] ([Ссылка]).

ПРАВИЛА:
1. Для КАЖДОГО товара из списка ниже предоставь актуальную цену.
2. Сохраняй ТОЧНОЕ название (title) и ссылку (link), которые предоставлены тебе. Это критично для обновления базы данных.
3. Если товара нет в списке, не выдумывай новые.
  ...
  "products": [
    {
      ...
      "price": "18500 ₽",
      "store": "DNS",
      "link": "оригинальная_ссылка"
    }
  ]
}
Важно: Только JSON. Актуализируй цены на ФЕВРАЛЬ 2026 года (учитывай дефицит и рост цен на DRAM).`;
...
Важно: Предлагай РЕАЛЬНО СУЩЕСТВУЮЩИЕ модели. В поле search_keywords пиши ТОЛЬКО ключевое название модели без лишних слов, но ОБЯЗАТЕЛЬНО включай критические характеристики для поиска: тип памяти (DDR4/DDR5), сокет (AM5/LGA1700), если они важны для совместимости. Только JSON без пояснений. 
СЕЙЧАС ФЕВРАЛЬ 2026 ГОДА. Цены очень высокие. Не предлагай топовое железо в низкий бюджет.`;
...
export async function generateHardwareData(
    ...
    try {
        ...
        // Формируем список товаров для режима обновления цен
        let productsList = 'нет';
        if (mode === 'prices' && existingProducts.length > 0) {
        ...
        const { text } = await generateText({
            ...
            system: systemPrompt,
            messages: [{
                role: 'user', content: mode === 'prices'
                    ? `Обнови цены для ${existingProducts.length} товаров в категории ${category}`
                    : `Сгенерируй ${count} товаров для категории ${category}`
            }],
            temperature: 0.4,
        });
